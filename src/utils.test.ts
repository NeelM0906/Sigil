...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "sigil-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "sigil-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "sigil-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "sigil-lid-b-"));
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/sigil")).toBe(path.resolve(os.homedir(), "sigil"));
  });

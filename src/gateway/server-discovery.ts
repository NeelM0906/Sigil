...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Sigil";
  if (/sigil/i.test(trimmed)) return trimmed;
  return `${trimmed} (Sigil)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SIGIL_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "sigil");
  ...
  const binCli = path.join(cwd, "bin", "sigil.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SIGIL_TAILNET_DNS?.trim();

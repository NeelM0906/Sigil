...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.SIGIL_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "sigil");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.sigil-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".sigil");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "sigil.json");
      ...
      process.env.SIGIL_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.SIGIL_CONFIG_PATH = prev.configPath;
        process.env.SIGIL_GATEWAY_TOKEN = prev.token;
        process.env.SIGIL_SKIP_CHANNELS = prev.skipChannels;
        process.env.SIGIL_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.SIGIL_SKIP_CRON = prev.skipCron;
        process.env.SIGIL_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.SIGIL_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-wizard-home-"));
    process.env.HOME = tempHome;
    delete process.env.SIGIL_STATE_DIR;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.SIGIL_STATE_DIR = prev.stateDir;
      process.env.SIGIL_CONFIG_PATH = prev.configPath;
      process.env.SIGIL_GATEWAY_TOKEN = prev.token;
      process.env.SIGIL_SKIP_CHANNELS = prev.skipChannels;
      process.env.SIGIL_SKIP_GMAIL_WATCHER = prev.skipGmail;

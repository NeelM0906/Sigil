...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `sigil-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.SIGIL_CONFIG_PATH = path.join(root, "sigil.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "sigil.json");

...
beforeAll(async () => {
  previousToken = process.env.SIGIL_GATEWAY_TOKEN;
...
afterAll(async () => {
  await server.close();
  if (previousToken === undefined) delete process.env.SIGIL_GATEWAY_TOKEN;
  else process.env.SIGIL_GATEWAY_TOKEN = previousToken;
});
...
describe("gateway server sessions", () => {
  ...
  test("lists and patches session store via sessions.* RPC", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-sessions-"));
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;

...
import { resolveSigilAgentDir } from "../agents/agent-paths.js";
...
import { ensureSigilModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { SigilConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.SIGIL_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SIGIL_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.SIGIL_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<SigilConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.SIGIL_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.SIGIL_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveSigilAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-live-state-"));
  process.env.SIGIL_STATE_DIR = tempStateDir;
  ...
  process.env.SIGIL_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.sigil-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSigilAgentDir();
  const sanitizedCfg: SigilConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Sigil live tool probe (local, safe): " +
    ...
    process.env.SIGIL_CONFIG_PATH = previous.configPath;
    process.env.SIGIL_GATEWAY_TOKEN = previous.token;
    process.env.SIGIL_SKIP_CHANNELS = previous.skipChannels;
    process.env.SIGIL_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.SIGIL_SKIP_CRON = previous.skipCron;
    process.env.SIGIL_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.SIGIL_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SIGIL_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureSigilModelsJson(cfg);
      ...
      const rawModels = process.env.SIGIL_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.sigil-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

...
import { createSigilTools } from "../agents/sigil-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.SIGIL_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.SIGIL_GATEWAY_PORT = String(gatewayPort);
  process.env.SIGIL_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SIGIL_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createSigilTools().find((candidate) => candidate.name === "sessions_send");

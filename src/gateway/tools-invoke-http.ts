...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-sigil-message-channel") ?? "");
  const accountId = getHeader(req, "x-sigil-account-id")?.trim() || undefined;
  ...
  const allTools = createSigilTools({

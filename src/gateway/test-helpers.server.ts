...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.SIGIL_STATE_DIR;
  previousConfigPath = process.env.SIGIL_CONFIG_PATH;
  previousSkipBrowserControl = process.env.SIGIL_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.SIGIL_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.SIGIL_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SIGIL_STATE_DIR = path.join(tempHome, ".sigil");
  delete process.env.SIGIL_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.SIGIL_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "sigil-test-"))
    : path.join(tempHome, ".sigil-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.SIGIL_STATE_DIR;
    else process.env.SIGIL_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.SIGIL_CONFIG_PATH;
    else process.env.SIGIL_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.SIGIL_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.SIGIL_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.SIGIL_SKIP_GMAIL_WATCHER;
    else process.env.SIGIL_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.SIGIL_SKIP_CANVAS_HOST;
    else process.env.SIGIL_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.SIGIL_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SIGIL_GATEWAY_TOKEN = fallbackToken;
  }

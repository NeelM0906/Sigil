...
import { ensureSigilCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.SIGIL_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.SIGIL_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureSigilCliOnPath({ pathEnv: process.env.PATH ?? "" });

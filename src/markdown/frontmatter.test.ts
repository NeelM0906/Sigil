...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { sigil?: { emoji?: string } };
    expect(parsed.sigil?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { sigil?: { events?: string[] } };

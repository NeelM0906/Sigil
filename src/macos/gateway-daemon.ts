...
const BUNDLED_VERSION =
  (typeof __SIGIL_VERSION__ === "string" && __SIGIL_VERSION__) ||
  process.env.SIGIL_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `sigil --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.SIGIL_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ?? process.env.SIGIL_GATEWAY_BIND ?? cfg.gateway?.bind ?? "loopback";
  const bind =
    bindRaw === "loopback" ||
    bindRaw === "lan" ||
    bindRaw === "auto" ||

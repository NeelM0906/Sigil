...
function candidateBinDirs(opts: EnsureSigilPathOpts): string[] {
  ...
  // Bundled macOS app: `sigil` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingSigil = path.join(execDir, "sigil");
    if (isExecutable(siblingSigil)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/sigil` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "sigil"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `sigil` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSigilCliOnPath(opts: EnsureSigilPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SIGIL_PATH_BOOTSTRAPPED)) return;
  process.env.SIGIL_PATH_BOOTSTRAPPED = "1";

...
export function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[sigil] Unhandled rejection handler failed:",
...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[sigil] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }
    ...
    if (isTransientNetworkError(reason)) {
      console.error("[sigil] Network error (non-fatal):", formatUncaughtError(reason));
      return;
    }

    console.error("[sigil] Unhandled promise rejection:", formatUncaughtError(reason));
    process.exit(1);
  });
}

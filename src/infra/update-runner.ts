...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "sigil doctor",
        managerScriptArgs(manager, "sigil", ["doctor", "--non-interactive"]),
        gitRoot,
        { SIGIL_UPDATE_IN_PROGRESS: "1" },
      ),
    );

...
export const WIDE_AREA_DISCOVERY_DOMAIN = "sigil.internal.";
export const WIDE_AREA_ZONE_FILENAME = "sigil.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*sigil-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "sigil";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "sigil");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "sigil-gw");
  ...
  records.push(`_sigil-gw._tcp IN PTR ${instanceLabel}._sigil-gw._tcp`);
  records.push(`${instanceLabel}._sigil-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._sigil-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; sigil-content-hash: ${contentHash}\n${contentBody}`;
}

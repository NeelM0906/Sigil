...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SIGIL_DISABLE_BONJOUR)) return true;
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Sigil";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Sigil\)\s*$/i, "").trim() || normalized;
}

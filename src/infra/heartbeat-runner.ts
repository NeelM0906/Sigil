...
function resolveActiveHoursTimezone(cfg: SigilConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SigilConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: SigilConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: SigilConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: SigilConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: SigilConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: SigilConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SigilConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function resolveHeartbeatSession(cfg: SigilConfig, agentId?: string, heartbeat?: HeartbeatConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SigilConfig) => {
    if (state.stopped) return;
    const now = Date.now();

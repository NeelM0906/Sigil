...
import { loadSigilPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.SIGIL_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `sigil-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "sigil.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SIGIL_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadSigilPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.SIGIL_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadSigilPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadSigilPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],

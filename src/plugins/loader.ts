...
import { discoverSigilPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SigilPluginDefinition;
  register?: SigilPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SigilPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SigilPluginDefinition;
...
export function loadSigilPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSigilPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "sigil/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SigilPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as SigilPluginModule;
    } catch (err) {

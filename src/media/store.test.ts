...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "SIGIL_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.SIGIL_STATE_DIR = path.join(home, ".sigil");
    ...
    await fs.mkdir(path.join(home, ".sigil"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.sigil${path.sep}media`);

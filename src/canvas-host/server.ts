...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.sigilCanvasA2UIAction ||
        window.webkit.messageHandlers.sigilCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.sigilCanvasA2UIAction &&
        typeof window.sigilCanvasA2UIAction.postMessage === "function") ||
      (window.sigilCanvasA2UIAction &&
        typeof window.sigilCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.sigilSendUserAction === "function" ? window.sigilSendUserAction : undefined;
  if (!window.sigilSendUserAction && legacySend) {
    window.sigilSendUserAction = legacySend;
  }
  if (!window.sigilSendUserAction && typeof window.sigilSendUserAction === "function") {
    window.sigilSendUserAction = window.sigilSendUserAction;
  }
  const hasHelper = () =>
    typeof window.sigilSendUserAction === "function" ||
    typeof window.sigilSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("sigil:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Sigil node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.sigilSendUserAction === "function"
        ? window.sigilSendUserAction
        : window.sigilSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SIGIL_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "sigil", "canvas"));
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Sigil Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;

...
import type { SigilConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: SigilConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: SigilConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SigilSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SigilConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SigilConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as SigilConfig)),
    ),
  };
}

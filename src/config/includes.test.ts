...
const ETC_SIGIL_DIR = path.join(ROOT_DIR, "etc", "sigil");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "sigil.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSigilPath(...parts: string[]) {
  return path.join(ETC_SIGIL_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcSigilPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "sigil.json"))).toEqual({
      shared: true,
    });
  });
});

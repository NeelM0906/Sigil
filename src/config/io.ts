...
import type { SigilConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareSigilVersions } from "./version.js";
...
function coerceConfig(value: unknown): SigilConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SigilConfig;
}
...
function stampConfigVersion(cfg: SigilConfig): SigilConfig {
...
function warnIfConfigFromFuture(cfg: SigilConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareSigilVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Sigil (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: SigilConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SigilConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as SigilConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SigilConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SigilConfig) {
    clearConfigCache();
...
// module scope. `SIGIL_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SIGIL_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SIGIL_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export async function writeConfigFile(cfg: SigilConfig): Promise<void> {
  clearConfigCache();

...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.SIGIL_WINDOWS_TASK_NAME?.trim();
  if (override) return override;
  return resolveGatewayWindowsTaskName(env.SIGIL_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.SIGIL_TASK_SCRIPT?.trim();
  if (override) return override;
  const scriptName = env.SIGIL_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export async function installScheduledTask({
  ...
  const taskDescription =
    ...
    formatGatewayServiceDescription({
      profile: env.SIGIL_PROFILE,
      version: environment?.SIGIL_SERVICE_VERSION ?? env.SIGIL_SERVICE_VERSION,
    });

...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "sigil-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".sigil", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<SigilConfig> = {},
): SigilConfig {
  const base: SigilConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".sigil", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {

...
export function listIMessageAccountIds(cfg: SigilConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: SigilConfig): string {
...
function mergeIMessageAccountConfig(cfg: SigilConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: SigilConfig): ResolvedIMessageAccount[] {

...
export function listSignalAccountIds(cfg: SigilConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: SigilConfig): string {
...
function mergeSignalAccountConfig(cfg: SigilConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: SigilConfig): ResolvedSignalAccount[] {

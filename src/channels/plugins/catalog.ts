...
import { discoverSigilPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { SigilPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  sigil?: SigilPackageManifest;
  [LEGACY_MANIFEST_KEY]?: SigilPackageManifest;
};
...
const ENV_CATALOG_PATHS = ["SIGIL_PLUGIN_CATALOG_PATHS", "SIGIL_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<SigilPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageSigil?: SigilPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageSigil;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.sigil ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageSigil: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverSigilPlugins({ workspaceDir: options.workspaceDir });

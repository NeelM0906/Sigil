...
function setSlackDmPolicy(cfg: SigilConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Sigil";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Sigil`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: SigilConfig, allowFrom: string[]): SigilConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,

...
const defaultTools = createSigilCodingTools();

describe("createSigilCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createSigilCodingTools();
    ...
    try {
      ...
      const contents = "Hello from sigil read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "sigil-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "sigil-workspace"),
    ...
    const tools = createSigilCodingTools({ sandbox });

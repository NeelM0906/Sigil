...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.SIGIL_AGENT_DIR;
    else process.env.SIGIL_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-chutes-"));
    process.env.SIGIL_STATE_DIR = tempDir;
    process.env.SIGIL_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");

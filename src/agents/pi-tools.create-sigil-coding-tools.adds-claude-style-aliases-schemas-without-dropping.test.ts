...
import { __testing, createSigilCodingTools } from "./pi-tools.js";
...
describe("createSigilCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSigilCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "sigil-outside.txt");

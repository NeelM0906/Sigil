...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "sigil.sandbox=1");
  args.push("--label", `sigil.sessionKey=${params.scopeKey}`);
  args.push("--label", `sigil.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `sigil.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "sigil.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`sigil sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`sigil sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("sigil sandbox recreate --all");
}

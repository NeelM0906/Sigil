...
type WebSearchConfig = NonNullable<SigilConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: SigilConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("sigil configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.sigil.bot/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://sigil.bot",
      "X-Title": "Sigil Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [

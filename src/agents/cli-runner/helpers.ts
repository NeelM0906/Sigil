...
/**
 * Cleanup suspended Sigil CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: SigilConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-cli-images-"));
  const paths: string[] = [];

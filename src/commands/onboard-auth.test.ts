...
const requireAgentDir = () => {
  const agentDir = process.env.SIGIL_AGENT_DIR;
  if (!agentDir) throw new Error("SIGIL_AGENT_DIR not set");
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.SIGIL_STATE_DIR;
  const previousAgentDir = process.env.SIGIL_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.SIGIL_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.SIGIL_AGENT_DIR;
    } else {
      process.env.SIGIL_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.SIGIL_OAUTH_DIR;
  });

  it("writes auth-profiles.json under SIGIL_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-oauth-"));
    process.env.SIGIL_STATE_DIR = tempStateDir;
    process.env.SIGIL_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SIGIL_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SIGIL_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-minimax-"));
    process.env.SIGIL_STATE_DIR = tempStateDir;
    process.env.SIGIL_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SIGIL_AGENT_DIR;

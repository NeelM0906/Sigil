...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidate = path.resolve(root, entry.name, ".sigil");
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".sigil");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.sigil out of git; it contains credentials and session history.",

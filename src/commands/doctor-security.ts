...
import type { SigilConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: SigilConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("sigil security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("sigil configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("sigil config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("sigil doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("sigil config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {

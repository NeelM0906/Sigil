...
function resolveDefaultAccountId(cfg: SigilConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: SigilConfig): boolean {
...
export function summarizeBindings(cfg: SigilConfig, bindings: AgentBinding[]): string[] {

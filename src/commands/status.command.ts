...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Sigil status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("sigil security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("sigil security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.sigil.bot/faq");
  runtime.log("Troubleshooting: https://docs.sigil.bot/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("sigil status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("sigil logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("sigil status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("sigil gateway probe")}`);
  }
}

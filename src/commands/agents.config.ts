...
export function listAgentEntries(cfg: SigilConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SigilConfig, agentId: string) {
...
function resolveAgentModel(cfg: SigilConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SigilConfig): AgentSummary[] {

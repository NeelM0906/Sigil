...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("sigil models auth setup-token")}\` or \`${formatCliCommand("sigil configure")}\`.`
          : `Run \`${formatCliCommand("sigil configure")}\` or set an API key env var.`;

...
import { readConfigFileSnapshot, type SigilConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: SigilConfig, model: string): SigilConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("sigil plugins install")}\`.`,
    );
  }

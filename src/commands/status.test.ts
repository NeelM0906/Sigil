...
beforeAll(() => {
  previousProfile = process.env.SIGIL_PROFILE;
  process.env.SIGIL_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.SIGIL_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/sigil-root.js", () => ({
  resolveSigilPackageRoot: vi.fn().mockResolvedValue("/tmp/sigil"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/sigil/pnpm-lock.yaml",
      markerPath: "/tmp/sigil/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.sigil.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.sigil.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Sigil status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("sigil status --all") ||
          l.includes("sigil --profile isolated status --all") ||
          l.includes("sigil status --all") ||
          l.includes("sigil --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.SIGIL_GATEWAY_TOKEN = "abcd1234";
    try {

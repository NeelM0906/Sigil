...
describe("doctor command", () => {
  ...
  it("warns when extra workspace directories exist", async () => {
    ...
    const legacyPath = path.join("/Users/steipete", "sigil");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (value === "/Users/steipete/sigil" || value === legacyPath || value === legacyAgentsPath)
        return true;
    ...
    expect(
      note.mock.calls.some(
        ([message, title]) =>
          title === "Extra workspace" && typeof message === "string" && message.includes("sigil"),
      ),
    ).toBe(true);

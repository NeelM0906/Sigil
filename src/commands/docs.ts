...
const SEARCH_TOOL = "https://docs.sigil.bot/mcp.SearchSigil";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.sigil.bot");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('sigil docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.sigil.bot/");
      runtime.log(`Search: ${formatCliCommand('sigil docs "your query"')}`);
    }
    return;
  }

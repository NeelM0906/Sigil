...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SIGIL_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SIGIL_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SIGIL_GATEWAY_TOKEN");
    expect(message).not.toContain("SIGIL_GATEWAY_PASSWORD");
  });

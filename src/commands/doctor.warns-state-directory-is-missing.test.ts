...
beforeEach(() => {
  ...
  resolveSigilPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SIGIL_STATE_DIR;
  originalUpdateInProgress = process.env.SIGIL_UPDATE_IN_PROGRESS;
  process.env.SIGIL_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "sigil-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SIGIL_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SIGIL_UPDATE_IN_PROGRESS;
  } else {
    process.env.SIGIL_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveSigilPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadSigilPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/sigil-root.js", () => ({
  resolveSigilPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "sigil-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.SIGIL_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SIGIL_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/sigil.json",
      exists: true,
      raw: "{}",
    ...
    process.env.SIGIL_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();

...
function stripUnknownConfigKeys(config: SigilConfig): {
  config: SigilConfig;
  removed: string[];
} {
  const parsed = SigilSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as SigilConfig;
...
function noteOpencodeProviderOverrides(cfg: SigilConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: SigilConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as SigilConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("sigil doctor --fix")}" to apply legacy migrations.`);
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("sigil doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "sigil doctor --fix" to remove these keys.');
    }
  }

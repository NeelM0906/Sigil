...
import { resolveSigilPackageRoot } from "../infra/sigil-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "sigil doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "sigil";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "sigil"]);
const CLI_NAME = resolveCliName();
const SIGIL_REPO_URL = "https://github.com/sigil/sigil.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "sigil");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("sigil@")) return trimmed.slice("sigil@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.SIGIL_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SIGIL_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SIGIL_GIT_DIR points at a non-git directory: ${params.dir}. Set SIGIL_GIT_DIR to an empty folder or a sigil checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SIGIL_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Sigil update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Sigil..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Sigil install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("sigil doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("sigil gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g sigil@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g sigil@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.SIGIL_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.SIGIL_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("sigil gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("sigil doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("sigil gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("sigil gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `sigil update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `SIGIL_GIT_DIR points at a non-git directory: ${gitDir}. Set SIGIL_GIT_DIR to an empty folder or a sigil checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via SIGIL_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Sigil to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["sigil update", "Update a source checkout (git)"],
        ["sigil update --channel beta", "Switch to beta channel (git + npm)"],
        ["sigil update --channel dev", "Switch to dev channel (git + npm)"],
        ["sigil update --tag beta", "One-off update to a dist-tag or version"],
        ["sigil update --no-restart", "Update without restarting the service"],
        ["sigil update --json", "Output result as JSON"],
        ["sigil update --yes", "Non-interactive (accept downgrade prompts)"],
        ["sigil update wizard", "Interactive update wizard"],
        ["sigil --update", "Shorthand for sigil update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run sigil update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.sigil.bot/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["sigil update status", "Show channel + version status."],
          ["sigil update status --json", "JSON output."],
          ["sigil update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.sigil.bot/cli/update")}`,
    )
    .action(async (opts) => {
      try {

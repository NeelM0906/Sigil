...
This directory contains hooks that ship with Sigil. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.sigil.bot/hooks/soul-evil
...
homepage: https://docs.sigil.bot/hooks#my-hook
metadata:
  { "sigil": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.sigil/sigil.json`:
...
2. Restart gateway: `pkill -9 -f 'sigil.*gateway' && pnpm sigil gateway`
3. Enable the hook: `sigil hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.sigil.bot/hooks

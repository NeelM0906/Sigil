...
homepage: https://docs.sigil.bot/hooks#command-logger
metadata:
  {
    "sigil":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.sigil/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.sigil/logs/commands.log ~/.sigil/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/sigil`:
   ```
   /home/username/.sigil/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok

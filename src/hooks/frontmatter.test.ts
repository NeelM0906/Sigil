...
`;
    ...
    expect(parsed.sigil.emoji).toBe("💾");
    expect(parsed.sigil.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.sigil.emoji).toBe("📝");
    expect(parsed.sigil.events).toEqual(["command"]);
    expect(parsed.sigil.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.sigil.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"sigil": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"sigil": {"events": ["test"]}}');
  });
...
describe("resolveSigilMetadata", () => {
  it("extracts sigil metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveSigilMetadata(frontmatter);
  ...
  it("returns undefined when sigil key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        sigil: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Sigil" },
            { id: "npm", kind: "npm", package: "@sigil/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@sigil/hook");
  });
...
`;
    ...
    const sigil = resolveSigilMetadata(frontmatter);
    expect(sigil).toBeDefined();
    expect(sigil?.emoji).toBe("💾");
    expect(sigil?.events).toEqual(["command:new"]);
    expect(sigil?.requires?.config).toEqual(["workspace.dir"]);
    expect(sigil?.install?.[0].kind).toBe("bundled");
  });

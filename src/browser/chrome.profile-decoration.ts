...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".sigil-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + sigil-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateSigilProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SIGIL_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SIGIL_BROWSER_COLOR).toUpperCase();

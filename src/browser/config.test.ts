...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const clawd = resolveProfile(resolved, "sigil");
    expect(clawd?.driver).toBe("sigil");
  ...
  it("derives default ports from SIGIL_GATEWAY_PORT when unset", () => {
    const prev = process.env.SIGIL_GATEWAY_PORT;
    process.env.SIGIL_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      if (prev === undefined) {
        ...
        process.env.SIGIL_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "sigil");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    ...
    expect(resolved.defaultProfile).toBe("sigil");
  });
});

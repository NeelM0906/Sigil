...
type AgentConfig = Partial<NonNullable<NonNullable<SigilConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.sigil/sessions/<SessionId>.jsonl)
...
const formatVoiceModeLine = (config?: SigilConfig, sessionEntry?: SessionEntry): string | null => {
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `✨ Sigil ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: SigilConfig): string {
  const lines = ["ℹ️ Help", ""];

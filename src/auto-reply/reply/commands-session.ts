...
import { scheduleGatewaySigusr1Restart, triggerSigilRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting sigil in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerSigilRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting sigil via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};

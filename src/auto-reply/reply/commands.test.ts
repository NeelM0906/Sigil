...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-commands-"));
...
function buildParams(commandBody: string, cfg: SigilConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,

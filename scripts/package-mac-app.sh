...
# Build and bundle Sigil into a minimal .app we can open.
# Outputs to dist/Sigil.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Sigil.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Sigil"
BUNDLE_ID="${BUNDLE_ID:-com.sigil.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/sigil/sigil/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Sigil/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SigilBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SigilGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Sigil"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Sigil"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Sigil"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Sigil" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Sigil/Resources/Sigil.icns" "$APP_ROOT/Contents/Resources/Sigil.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Sigil/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SigilKit resources"
SIGILKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SigilKit_SigilKit.bundle"
if [ -d "$SIGILKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SigilKit_SigilKit.bundle"
  cp -R "$SIGILKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SigilKit_SigilKit.bundle"
else
  echo "WARN: SigilKit resource bundle not found at $SIGILKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Sigil"
killall -q Sigil 2>/dev/null || true

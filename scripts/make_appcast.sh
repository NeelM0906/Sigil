...
ZIP=${1:?"Usage: $0 Sigil-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/sigil/sigil/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Sigil-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/sigil/sigil/releases/download/v${VERSION}/"}

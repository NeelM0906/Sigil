...
BASE_IMAGE="${BASE_IMAGE:-sigil-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-sigil-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=sigil.sandbox=1)
NOTE

...
IMAGE_NAME="sigil-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="sigil-net-e2e-$$"
GW_NAME="sigil-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SIGIL_GATEWAY_TOKEN=$TOKEN" \
	  -e "SIGIL_SKIP_CHANNELS=1" \
	  -e "SIGIL_SKIP_GMAIL_WATCHER=1" \
	  -e "SIGIL_SKIP_CRON=1" \
	  -e "SIGIL_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \

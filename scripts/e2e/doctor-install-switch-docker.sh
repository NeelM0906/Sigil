...
IMAGE_NAME="sigil-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/sigil-bin:$PATH"
  mkdir -p /tmp/sigil-bin

  cat > /tmp/sigil-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/sigil-bin/systemctl

  cat > /tmp/sigil-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/sigil-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/sigil"
  npm_entry="/tmp/npm-prefix/lib/node_modules/sigil/dist/entry.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/sigil-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/sigil-gateway.service"
    if [ ! -f "$unit_path" ]; then

...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/sigil-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/sigil-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/sigil-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.sigil/sigil.json"
    sessions_dir="$HOME/.sigil/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.sigil/sigil.json" <<'"'"'JSON'"'"'
{

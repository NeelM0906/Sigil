...
#
# Suggested cron: */30 * * * * /home/admin/sigil/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Sigil notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., sigil-alerts)
...
STATE_FILE="$HOME/.sigil/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Sigil if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use sigil
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Sigil to $NOTIFY_PHONE..."
            sigil send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Sigil is down. Run: ssh l36 '~/sigil/scripts/mobile-reauth.sh'" "urgent"
    exit 1

...
const compiler = env.SIGIL_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.SIGIL_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.SIGIL_RUNNER_LOG === "0") return;
  process.stderr.write(`[sigil] ${message}\n`);
};

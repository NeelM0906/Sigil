#!/bin/bash
# Setup Sigil Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'sigil-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Sigil message: Send warning via Sigil itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/sigil-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/sigil-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/sigil-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now sigil-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status sigil-auth-monitor.timer"
echo ""
echo "Quick commands:"

...
docker run --rm -t \
  ...
  -e SIGIL_LIVE_TEST=1 \
  -e SIGIL_LIVE_GATEWAY_MODELS="${SIGIL_LIVE_GATEWAY_MODELS:-all}" \
  -e SIGIL_LIVE_GATEWAY_PROVIDERS="${SIGIL_LIVE_GATEWAY_PROVIDERS:-}" \
  -e SIGIL_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${SIGIL_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}" \

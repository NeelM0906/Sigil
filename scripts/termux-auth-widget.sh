...
SERVER="${SIGIL_SERVER:-l36}"

# Check auth status
termux-toast "Checking Sigil auth..."

STATUS=$(ssh "$SERVER" '$HOME/sigil/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/sigil/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|SIGIL_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/sigil/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Sigil won't work until re-auth"
                ;;
        esac
        ;;

...
import { SigilApp } from "./app";
import "../styles.css";

const originalConnect = SigilApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("sigil-app") as SigilApp;
...
beforeEach(() => {
  SigilApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__SIGIL_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  SigilApp.prototype.connect = originalConnect;
  window.__SIGIL_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/sigil/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/sigil");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/sigil/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SIGIL_CONTROL_UI_BASE_PATH__ = "/sigil";
    const app = mountApp("/sigil/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/sigil");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/sigil/sessions");
  });

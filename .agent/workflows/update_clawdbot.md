---
description: Update Sigil from upstream when branch has diverged (ahead/behind)
---

# Sigil Upstream Sync Workflow
...
# Move old version
mv /Applications/Sigil.app /tmp/Sigil-backup.app

# Install new build
cp -R dist/Sigil.app /Applications/
...
ps aux | grep -E "(sigil|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm sigil agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update sigil.json with fallback chains
...
pnpm sigil agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"

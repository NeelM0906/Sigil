...
This document describes how the Sigil ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`sigil acp` exposes an ACP agent over stdio and forwards prompts to a running
Sigil Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Sigil Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `sigil acp` over stdio.

Example config:

```bash
sigil config set gateway.remote.url wss://gateway-host:18789
sigil config set gateway.remote.token <token>
```

Example run:

```bash
sigil acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
sigil acp --session agent:main:main
sigil acp --session agent:design:main
sigil acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Sigil ACP” to start a thread.

## Execution Model

- ACP client spawns `sigil acp` and speaks ACP messages over stdio.
...
```bash
sigil acp --session agent:main:main
sigil acp --session-label "support inbox"
sigil acp --reset-session
```
...
`sigil acp` resolves the Gateway URL and auth from CLI flags or config:

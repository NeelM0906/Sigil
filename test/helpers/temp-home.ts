...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SIGIL_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.SIGIL_STATE_DIR = path.join(base, ".sigil");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "sigil-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".sigil", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {

...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `sigil-e2e-${name}-`));
  const configDir = path.join(homeDir, ".sigil");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          SIGIL_CONFIG_PATH: configPath,
          SIGIL_STATE_DIR: stateDir,
          SIGIL_GATEWAY_TOKEN: "",
          SIGIL_GATEWAY_PASSWORD: "",
          SIGIL_SKIP_CHANNELS: "1",
          SIGIL_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        SIGIL_GATEWAY_TOKEN: inst.gatewayToken,
        SIGIL_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SIGIL_GATEWAY_PORT: String(gwA.port),
          SIGIL_GATEWAY_TOKEN: gwA.gatewayToken,
          SIGIL_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SIGIL_GATEWAY_PORT: String(gwB.port),
          SIGIL_GATEWAY_TOKEN: gwB.gatewayToken,
          SIGIL_GATEWAY_PASSWORD: "",
        }),

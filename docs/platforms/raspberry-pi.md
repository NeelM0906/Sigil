---
summary: "Sigil on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Sigil on a Raspberry Pi
  - Running Sigil on ARM devices
  - Building a cheap always-on personal AI
---

# Sigil on Raspberry Pi

## Goal

Run a persistent, always-on Sigil Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Sigil features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled sigil

# Enable if not
sudo systemctl enable sigil
...
```bash
# Check logs
journalctl -u sigil --no-pager -n 100

# Common fix: rebuild
cd ~/sigil  # if using hackable install
npm run build
sudo systemctl restart sigil
```

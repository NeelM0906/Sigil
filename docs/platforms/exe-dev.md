---
summary: "Run Sigil Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: Sigil Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install Sigil  
2) Run `sigil onboard --install-daemon`  
...
Tip: keep this VM **stateful**. Sigil stores state under `~/.sigil/` and `~/sigil/`.
...
```bash
ssh -N -L 18789:127.0.0.1:18789 sigil.exe.xyz
```
...
```bash
export SIGIL_GATEWAY_TOKEN="$(openssl rand -hex 32)"
sigil gateway --bind lan --port 8080 --token "$SIGIL_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.sigil/sigil.json`:
...
On Linux, Sigil uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status sigil-gateway[-<profile>].service
```

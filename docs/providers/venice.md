---
summary: "Use Venice AI privacy-focused models in Sigil"
read_when:
  - You want privacy-focused inference in Sigil
...
```bash
sigil onboard --auth-choice venice-api-key
```
...
```bash
sigil chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Sigil shows all available Venice models. Pick based on your needs:
...
```bash
sigil models set venice/claude-opus-45
sigil models set venice/llama-3.3-70b
```

List all available models:

```bash
sigil models list | grep venice
```

## Configure via `sigil configure`

1. Run `sigil configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Sigil automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
sigil chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
sigil chat --model venice/claude-opus-45

# Use uncensored model
sigil chat --model venice/venice-uncensored

# Use vision model with image
sigil chat --model venice/qwen3-vl-235b-a22b

# Use coding model
sigil chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `sigil models list` to see currently available models. Some models may be temporarily offline.

---
summary: "Use Synthetic's Anthropic-compatible API in Sigil"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Sigil registers it as the
...
```bash
sigil onboard --auth-choice synthetic-api-key
```
...
Note: Sigil's Anthropic client appends `/v1` to the base URL, so use

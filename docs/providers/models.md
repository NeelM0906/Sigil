---
summary: "Model providers (LLMs) supported by Sigil"
read_when:
...
Sigil can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `sigil onboard`).

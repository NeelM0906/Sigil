...
Sigil can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Sigil can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Sigil defaults
...
TTS config lives under `messages.tts` in `sigil.json`.
...
- If `provider` is **unset**, Sigil prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.sigil/settings/tts.json`, override with `SIGIL_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Sigil retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Sigil registers

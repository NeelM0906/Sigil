...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Sigil
---

# Node + tsx "__name is not a function" crash

## Summary
Running Sigil via Node with `tsx` fails at startup with:

```
[sigil] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `sigil status` and `gateway:watch` worked.

...
read_when:
  - Designing workspace memory (~/sigil) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Sigil integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/sigil`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/sigil` as canonical human-readable memory.
...
Recommendation: **deep integration in Sigil**, but keep a separable core library.

### Why integrate into Sigil?
- Sigil already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `sigil memory recall "…" --k 25 --since 30d`
  - `sigil memory reflect --since 7d`

...
Sigil features that can generate provider usage or paid API calls.
...
- `sigil status --usage` and `sigil channels list` show provider **usage windows**
...
Sigil can pick up credentials from:
...
### 8) Model scan / probe
`sigil models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.

---
summary: "First-run onboarding flow for Sigil (macOS app)"
read_when:
...
- Writes credentials to `~/.sigil/credentials/oauth.json`
...
The app can install the global `sigil` CLI via npm/pnpm so terminal
...
On the first agent run, Sigil bootstraps a workspace (default `~/sigil`):
...
```bash
sigil webhooks gmail setup --account you@gmail.com
```

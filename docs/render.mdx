...
Deploy Sigil on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/sigil/sigil" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run Sigil:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SIGIL_STATE_DIR
        value: /data/.sigil
      - key: SIGIL_WORKSPACE_DIR
        value: /data/workspace
      - key: SIGIL_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/sigil`.
...
If you use the original Sigil repository, Render will not auto-deploy your Sigil. To update it, run a manual Blueprint sync from the dashboard.
...
For Sigil, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Sigil host.

...
You can also set `SIGIL_VERBOSE=1` instead of the flag.
...
### `docs.sigil.bot` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.sigil.bot` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.sigil.bot` to the allowlist, then retry.
...
- Check the allowlist: `sigil config get agents.defaults.models`
...
If you can, include the relevant log tail from `sigil logs --follow`.

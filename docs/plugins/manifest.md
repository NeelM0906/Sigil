...
read_when:
  - You are building a Sigil plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (sigil.plugin.json)

Every plugin **must** ship a `sigil.plugin.json` file in the **plugin root**.
Sigil uses this manifest to validate configuration **without executing plugin

...
Sigil controls a **dedicated Chrome profile** (named `clawd`, orange‑tinted UI). This is separate from your daily browser profile.
...
```bash
sigil browser start
sigil browser open https://x.com
```
...
```bash
sigil browser open https://x.com --browser-profile sigil --target host
```

...
- **gateway host** → `sigil` process on the gateway machine
...
`~/.sigil/exec-approvals.json` directly.

CLI: `sigil approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).

---
summary: "Chrome extension: let Sigil drive your existing Chrome tab"
read_when:
...
The Sigil Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate sigil-managed Chrome profile.
...
Sigil then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Sigil release (npm package) as static files. There is no separate “build” step.

After upgrading Sigil:
- Re-run `sigil browser extension install` to refresh the installed files under your Sigil state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Sigil ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Sigil to control.
...
- `ON`: attached; Sigil can drive that tab.
...
Debugging: `sigil sandbox explain`
...
`sigil browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `sigil browser extension install` first to copy the extension to a stable location under your Sigil state directory.

---
summary: "Direct `sigil agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `sigil agent` (direct agent runs)

`sigil agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `sigil message --target`).
...
```bash
sigil agent --to +15555550123 --message "status update"
sigil agent --agent ops --message "Summarize logs"
...
sigil agent --to +15555550123 --message "Summon reply" --deliver

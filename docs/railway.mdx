...
Deploy Sigil on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/sigil-railway-template" target="_blank" rel="noreferrer">Deploy on Railway</a>
...
- `https://<your-railway-domain>/sigil` — Control UI

## What you get

- Hosted Sigil Gateway + Control UI
...
- `SIGIL_STATE_DIR=/data/.sigil` (recommended)
- `SIGIL_WORKSPACE_DIR=/data/workspace` (recommended)
- `SIGIL_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Sigil state + workspace so you can migrate to another host without losing config or memory.

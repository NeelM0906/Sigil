...
```bash
sigil plugins install @sigil/msteams
```

Local checkout (when running from a git repo):
```bash
sigil plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Sigil will offer the local install path automatically.
...
3) Configure Sigil with those credentials.
...
## Goals
- Talk to Sigil via Teams DMs, group chats, or channels.
...
- On startup, Sigil resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.sigil/sigil.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Sigil, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `sigil-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `sigil plugins install @sigil/msteams`
   - From a local checkout: `sigil plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Sigil in Teams", "full": "Sigil in Teams" },
...
Sigil handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Sigil only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SigilShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
Sigil sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `sigil message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.sigil/msteams-polls.json`.
...
**CLI:**
```bash
sigil message send --channel msteams \
...
sigil message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
sigil message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
sigil message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
sigil message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```

...
```bash
sigil plugins install @sigil/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
sigil plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Sigil" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `sigil pairing list nextcloud-talk`
  - `sigil pairing approve nextcloud-talk <CODE>`

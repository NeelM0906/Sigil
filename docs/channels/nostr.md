...
read_when:
  - You want Sigil to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Sigil to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`sigil onboard`) and `sigil channels add` list optional channel plugins.
...
```bash
sigil plugins install @sigil/nostr
```

Use a local checkout (dev workflows):

```bash
sigil plugins install --link <path-to-sigil>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "sigil@example.com",
        "lud16": "sigil@example.com"
      }
    }
  }
}

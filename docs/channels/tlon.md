...
Tlon is a decentralized messenger built on Urbit. Sigil connects to your Urbit ship and can
...
```bash
sigil plugins install @sigil/tlon
```

Local checkout (when running from a git repo):

```bash
sigil plugins install ./extensions/tlon
```
...
Use these with `sigil message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Sigil replies in-thread.

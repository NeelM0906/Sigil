...
read_when:
  - Setting up Twitch chat integration for Sigil
---
# Twitch (plugin)

Twitch chat support via IRC connection. Sigil connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
sigil plugins install @sigil/twitch
```

Local checkout (when running from a git repo):

```bash
sigil plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `SIGIL_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "sigil",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use SIGIL_TWITCH_ACCESS_TOKEN env var)
...
```bash
SIGIL_TWITCH_ACCESS_TOKEN=oauth:abc123...
```

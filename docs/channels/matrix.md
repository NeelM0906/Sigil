...
Matrix is an open, decentralized messaging protocol. Sigil connects as a Matrix **user**
...
```bash
sigil plugins install @sigil/matrix
```

Local checkout (when running from a git repo):

```bash
sigil plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `sigil plugins install @sigil/matrix`
   - From a local checkout: `sigil plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Sigil calls the same
     login endpoint, stores the access token in `~/.sigil/credentials/matrix/credentials.json`,
...
- On first connection, Sigil requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Sigil logs a warning.
...
`~/.sigil/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `sigil pairing list matrix`
  - `sigil pairing approve matrix <CODE>`
...
- On startup, Sigil resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.

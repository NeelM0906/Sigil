...
- Jobs persist under `~/.sigil/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.sigil/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.sigil/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
sigil cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
sigil cron edit <jobId> --agent ops
sigil cron edit <jobId> --clear-agent
```
```
...
```bash
sigil system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`sigil system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `SIGIL_SKIP_CRON`.

...
- `/usage cost` in chats: local cost summary aggregated from Sigil session logs.
- CLI: `sigil status --usage` prints a full per-provider breakdown.
- CLI: `sigil channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

...
Delivery:
- Publish as `@sigil/plugin-sdk` (or export from core under `sigil/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `SigilPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SigilConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SigilConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SigilConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: SigilConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SigilConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SigilConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@sigil/plugin-sdk`.
- Add `api.runtime` to `SigilPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `sigilRuntime: ">=2026.2.0"`).

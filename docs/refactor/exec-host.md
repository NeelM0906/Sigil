...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.sigil/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.sigil/node.json` (node id + pairing token).
...
Path: `~/.sigil/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.sigil/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.sigil/exec-approvals.sock` (0600).

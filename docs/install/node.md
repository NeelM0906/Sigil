...
read_when:
  - "You installed Sigil but `sigil` is “command not found”"
...
Sigil’s runtime baseline is **Node 22+**.

If you can run `npm install -g sigil@latest` but later see `sigil: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.

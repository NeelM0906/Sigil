...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Sigil would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Sigil would reply in this chat, it will run all three agents.
...
**Debug:**
```bash
tail -f ~/.sigil/logs/gateway.log | grep broadcast
```

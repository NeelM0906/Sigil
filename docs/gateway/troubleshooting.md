---
summary: "Quick troubleshooting guide for common Sigil failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Sigil misbehaves, here's how to fix it.
...
|---|---|---|
| `sigil status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `sigil status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `sigil status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `sigil gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `sigil channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `sigil gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `sigil logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `sigil status --all` (it redacts tokens). If you paste `sigil status`, consider setting `SIGIL_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  sigil models auth setup-token --provider anthropic
  ```
...
```bash
sigil models auth paste-token --provider anthropic
sigil models status
```
...
**Logs:**
- Preferred: `sigil logs --follow`
- File logs (always): `/tmp/sigil/sigil-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$SIGIL_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u sigil-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  sigil config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  sigil config set gateway.mode remote
  sigil config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `sigil setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.sigil/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `sigil doctor`
...
- then run `sigil sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `sigil gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `sigil gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `sigil gateway status` and `sigil doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `SIGIL_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `sigil gateway status` shows a config mismatch**
...
- Fix: rerun `sigil gateway install --force` from the same `--profile` / `SIGIL_STATE_DIR` you want the service to use.

**If `sigil gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `sigil doctor` to update it (or `sigil gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `SIGIL_GATEWAY_TOKEN`) and restart the service.

**If `sigil gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/sigil` on disk.
...
Symptoms: `pwd` or file tools show `~/.sigil/sandboxes/...` even though you
...
Sigil intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `sigil models list` or
  `sigil models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${SIGIL_CONFIG_PATH:-$HOME/.sigil/sigil.json}"
```
...
tail -f "$(ls -t /tmp/sigil/sigil-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
sigil logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.sigil/agents/<agentId>/sessions/
```
...
sigil logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
sigil gateway --verbose
```

If you’re logged out / unlinked:

```bash
sigil channels logout
trash "${SIGIL_STATE_DIR:-$HOME/.sigil}/credentials" # if logout can't cleanly remove everything
sigil channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/sigil/sigil-*.log | head -1)" | tail -20
```

### High Memory Usage

Sigil keeps conversation history in memory.
...
Sigil now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `sigil doctor` reports every invalid entry.
- `sigil doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `sigil logs`, `sigil health`, `sigil status`, `sigil gateway status`, and `sigil gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/sigil/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `sigil models status` (CLI).
...
5) Run `sigil channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Sigil scrubs/normalizes tool
...
```bash
tccutil reset All com.sigil.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/sigil/sigil/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
sigil gateway status
sigil gateway stop
# Or: launchctl bootout gui/$UID/com.sigil.gateway (replace with com.sigil.<profile> if needed)
```
...
Ensure the global `sigil` CLI is installed and matches the app version:
```bash
sigil --version
...
#   ${SIGIL_CONFIG_PATH:-$HOME/.sigil/sigil.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
sigil gateway --verbose
sigil channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/sigil/sigil-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$SIGIL_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.sigil/logs/...`; profiles use `~/.sigil-<profile>/logs/...`)<br />Linux: `journalctl --user -u sigil-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Sigil Gateway (<profile>)" /V /FO LIST` |
| Session files | `$SIGIL_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$SIGIL_STATE_DIR/media/` |
| Credentials | `$SIGIL_STATE_DIR/credentials/` |
...
# sigil gateway uninstall

trash "${SIGIL_STATE_DIR:-$HOME/.sigil}"
sigil channels login         # re-pair WhatsApp
sigil gateway restart           # or: sigil gateway
```
...
1. Check logs first: `/tmp/sigil/` (default: `sigil-YYYY-MM-DD.log`, or your configured `logging.file`)

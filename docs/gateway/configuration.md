---
summary: "All configuration options for ~/.sigil/sigil.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Sigil reads an optional **JSON5** config from `~/.sigil/sigil.json` (comments + trailing commas allowed).

If the file is missing, Sigil uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/sigil`). You usually only need a config to:
...
Sigil only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `sigil doctor`, `sigil logs`, `sigil health`, `sigil status`, `sigil service`, `sigil help`).
- Run `sigil doctor` to see the exact issues.
- Run `sigil doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `sigil config set`. Keep a backup of `~/.sigil/sigil.json`.
...
```bash
sigil gateway call config.get --params '{}' # capture payload.hash
sigil gateway call config.apply --params '{
...
```bash
sigil gateway call config.get --params '{}' # capture payload.hash
sigil gateway call config.patch --params '{
...
{ "$include": "/etc/sigil/base.json5" } // absolute
...
```json5
// ~/.sigil/clients/mueller/agents.json5
[
...
```json5
// ~/.sigil/clients/mueller/broadcast.json5
{
...
Sigil reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Sigil runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `SIGIL_LOAD_SHELL_ENV=1`
- `SIGIL_SHELL_ENV_TIMEOUT_MS=15000`
...
Sigil stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.sigil/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.sigil/credentials/oauth.json` (or `$SIGIL_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.sigil/agent/*` (migrated by `sigil doctor` into `~/.sigil/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `SIGIL_OAUTH_DIR`
- Agent dir (default agent root override): `SIGIL_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Sigil imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Sigil derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/sigil/sigil-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/sigil/sigil.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.sigil/credentials/whatsapp/biz
          // authDir: "~/.sigil/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `sigil doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "sigil", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/sigil-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.sigil/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/sigil-home" },
      { id: "work", workspace: "~/sigil-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `sigil.json`).
...
Sigil starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
Sigil starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `sigil plugins install @sigil/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Sigil starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Sigil spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Sigil
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/sigil" } }
}
```
...
When a file exceeds this limit, Sigil logs a warning and injects a truncated
...
message envelopes). If unset, Sigil uses the host timezone at runtime.
...
`[sigil]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[sigil]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[sigil]"`, Sigil will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Sigil generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.sigil/settings/tts.json",
      elevenlabs: {
...
Sigil also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Sigil currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `sigil configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:sigil`: all built-in Sigil tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.sigil/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.sigil/sandboxes",
        docker: {
          image: "sigil-sandbox:bookworm-slim",
          containerPrefix: "sigil-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "sigil-sandbox-browser:bookworm-slim",
          containerPrefix: "sigil-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Sigil uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.sigil/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Sigil config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Sigil writes/merges a `models.json` into
`~/.sigil/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Sigil uses
...
- Shortcut: `sigil onboard --auth-choice opencode-zen`.
...
Shortcut: `sigil onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `sigil onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `sigil onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `sigil onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `sigil onboard --auth-choice minimax-api`.
...
- Override the agent config root with `SIGIL_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.sigil/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.sigil/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.sigil/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Sigil stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.sigil/skills` (workspace skills
...
from `~/.sigil/extensions`, `<workspace>/.sigil/extensions`, plus any
...
### `browser` (sigil-managed browser)

Sigil can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for the agent and expose a small loopback control service.
...
- profile color: `#FF4500` (sigil-orange)
- Note: the control server is started by the running gateway (Sigil.app menubar, or `sigil gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      sigil: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/sigil" }
...
- When a connection comes from one of these IPs, Sigil uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `sigil gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `SIGIL_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `SIGIL_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Sigil
...
macOS app behavior:
- Sigil.app watches `~/.sigil/sigil.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.sigil/sigil.json` (or `SIGIL_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.sigil/sigil.json` (or `SIGIL_CONFIG_PATH`)
...
- `SIGIL_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `sigil --dev …` → uses `~/.sigil-dev` + shifts ports from base `19001`
- `sigil --profile <name> …` → uses `~/.sigil-<name>` (port via config/env/flags)
...
Example:
```bash
SIGIL_CONFIG_PATH=~/.sigil/a.json \
SIGIL_STATE_DIR=~/.sigil-a \
...
Gmail helper config (used by `sigil webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "sigil@gmail.com",
...
- Set `SIGIL_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Sigil defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__sigil/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__sigil__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.sigil/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.sigil/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.sigil/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_sigil-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_sigil-bridge._tcp` under `~/.sigil/dns/` using the standard discovery domain `sigil.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `sigil.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `sigil.internal` via that server

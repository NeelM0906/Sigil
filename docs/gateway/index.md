...
- Replaces the legacy `gateway` command. CLI entry point: `sigil gateway`.
...
pnpm gateway:watch
```
- Config hot reload watches `~/.sigil/sigil.json` (or `SIGIL_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__sigil__/canvas/` from `~/sigil/canvas`. Disable with `canvasHost.enabled=false` or `SIGIL_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `SIGIL_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `SIGIL_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.sigil.<profile>`
- Linux: `sigil-gateway-<profile>.service`
- Windows: `Sigil Gateway (<profile>)`

Install metadata is embedded in the service config:
- `SIGIL_SERVICE_MARKER=sigil`
- `SIGIL_SERVICE_KIND=gateway`
- `SIGIL_SERVICE_VERSION=<version>`
...
```bash
sigil --dev setup
sigil --dev gateway --allow-unconfigured
...
- `SIGIL_STATE_DIR=~/.sigil-dev`
- `SIGIL_CONFIG_PATH=~/.sigil-dev/sigil.json`
- `SIGIL_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/sigil-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `SIGIL_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `SIGIL_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `SIGIL_CONFIG_PATH`
- unique `SIGIL_STATE_DIR`
...
Example:
```bash
SIGIL_CONFIG_PATH=~/.sigil/a.json SIGIL_STATE_DIR=~/.sigil-a sigil gateway --port 19001
SIGIL_CONFIG_PATH=~/.sigil/b.json SIGIL_STATE_DIR=~/.sigil-b sigil gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `sigil health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `sigil`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `sigil gateway install` writes `~/Library/LaunchAgents/com.sigil.gateway.plist`
    (or `com.sigil.<profile>.plist`).
  - `sigil doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Sigil profile services.
  ...
  - Cleanup: `sigil gateway uninstall` (current service) and `sigil doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `sigil gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Sigil.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.sigil.gateway` (or `com.sigil.<profile>`).
- To stop it cleanly, use `sigil gateway stop` (or `launchctl bootout gui/$UID/com.sigil.gateway`).
- To restart, use `sigil gateway restart` (or `launchctl kickstart -k gui/$UID/com.sigil.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `sigil gateway install` first.
  - Replace the label with `com.sigil.<profile>` when running a named profile.

## Supervision (systemd user unit)
Sigil installs a **systemd user service** by default on Linux/WSL2. We
...
`sigil gateway install` writes the user unit. `sigil doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/sigil-gateway[-<profile>].service`:
```
[Unit]
Description=Sigil Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/sigil gateway --port 18789
Restart=always
RestartSec=5
Environment=SIGIL_GATEWAY_TOKEN=
...
```
systemctl --user enable --now sigil-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/sigil-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now sigil-gateway[-<profile>].service
```
...
## CLI helpers
- `sigil gateway health|status` — request health/status over the Gateway WS.
- `sigil message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `sigil agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `sigil gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `sigil gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `sigil gateway` and the legacy TCP control port.

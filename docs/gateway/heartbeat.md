...
- During heartbeat runs, Sigil treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `sigil sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Sigil skips the heartbeat run entirely (no model call).
...
```bash
sigil system event --text "Check for urgent follow-ups" --mode now
```

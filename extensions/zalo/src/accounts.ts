...
export function listZaloAccountIds(cfg: SigilConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: SigilConfig): string {
...
function mergeZaloAccountConfig(cfg: SigilConfig, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: SigilConfig): ResolvedZaloAccount[] {

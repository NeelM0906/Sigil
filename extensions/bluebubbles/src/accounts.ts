...
export function listBlueBubblesAccountIds(cfg: SigilConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: SigilConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: SigilConfig): ResolvedBlueBubblesAccount[] {

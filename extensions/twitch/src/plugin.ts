/**
 * Twitch channel plugin for Sigil.
 *
...
import { buildChannelConfigSchema } from "sigil/plugin-sdk";
 ...
 * for Sigil. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SigilConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SigilConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SigilConfig): boolean => {

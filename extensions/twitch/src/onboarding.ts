...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: SIGIL_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: SigilConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var SIGIL_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => {
    ...
    return setTwitchAccessControl(cfg as SigilConfig, allowedRoles, true);
  },
  promptAllowFrom: async ({ cfg, prompter }) => {
    ...
    return setTwitchAccount(cfg as SigilConfig, {
      ...(account ?? undefined),
      allowFrom,
    });
  },
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.SIGIL_TWITCH_ACCESS_TOKEN?.trim();

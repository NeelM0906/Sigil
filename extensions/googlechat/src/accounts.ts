...
export function listGoogleChatAccountIds(cfg: SigilConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: SigilConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: SigilConfig): ResolvedGoogleChatAccount[] {

...
Sigil extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
sigil channels login --channel zalouser
...
```bash
sigil message send --channel zalouser --target <threadId> --message "Hello from Sigil!"
```
...
```bash
sigil channels login --channel zalouser              # Login via QR
sigil channels login --channel zalouser --account work
sigil channels status --probe
sigil channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
sigil directory self --channel zalouser
sigil directory peers list --channel zalouser --query "name"
sigil directory groups list --channel zalouser --query "work"
sigil directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
sigil message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
sigil message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `sigil channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
sigil channels login --channel zalouser --account work
sigil message send --channel zalouser --account work --target <id> --message "Hello"

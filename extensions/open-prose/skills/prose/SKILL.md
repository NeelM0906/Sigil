...
metadata: {"sigil":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Sigil `sessions_spawn`
- **File I/O** == Sigil `read`/`write`
- **Remote fetch** == Sigil `web_fetch` (or `exec` with curl when POST is required)

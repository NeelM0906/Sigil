...
// NOTE: This extension is intended to be bundled with Sigil.
// When running from source (tests/dev), Sigil internals live under src/.
...
export function createLlmTaskTool(api: SigilPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sigil-llm-task-"));

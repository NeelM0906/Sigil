...
`clawd.invoke --each`) without adding new Sigil code per workflow.
...
This extension depends on Sigil internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Sigil extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.sigil/extensions` as a standalone plugin directory.

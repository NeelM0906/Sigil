...
function resolveSigilRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.SIGIL_ROOT?.trim() || process.env.SIGIL_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set SIGIL_ROOT (or legacy SIGIL_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveSigilRoot();

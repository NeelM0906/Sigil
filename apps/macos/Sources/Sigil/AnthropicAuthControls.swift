...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SigilOAuthStore.AnthropicOAuthStatus = SigilOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: SigilOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = SigilOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SigilOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try SigilOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Sigil can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SigilOAuthStore.AnthropicOAuthStatus,

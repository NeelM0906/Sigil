...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "com.sigil", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SigilSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SigilSessionPreviewEntry,
    ...
    private static func cache(payload: SigilSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SigilChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SigilKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SigilKit.AnyCodable]) -> [SigilChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SigilChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SigilChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SigilChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SigilChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SigilChatMessage) -> String? {

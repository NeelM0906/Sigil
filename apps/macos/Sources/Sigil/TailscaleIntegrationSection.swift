...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.sigil/sigil.json. Prefer SIGIL_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.sigil/sigil.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.sigil/sigil.json. Restart the gateway to apply."
        }

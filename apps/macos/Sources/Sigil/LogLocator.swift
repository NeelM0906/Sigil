...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SIGIL_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/sigil")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("sigil-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("sigil-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/sigil/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("sigil") && $0.pathExtension == "log" }
            .max { lhs, rhs in

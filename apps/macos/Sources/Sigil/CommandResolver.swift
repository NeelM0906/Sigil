...
enum CommandResolver {
    private static let projectRootDefaultsKey = "sigil.gatewayProjectRootPath"
    private static let helperName = "sigil"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/sigil.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/sigil")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let sigilPaths = self.sigilManagedPaths(home: home)
        if !sigilPaths.isEmpty {
            extras.insert(contentsOf: sigilPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + sigilPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func sigilManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".sigil")
    ...
    static func sigilExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSigilExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/sigil.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnySigilInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.sigilExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let sigilPath = self.projectSigilExecutable(projectRoot: root) {
                return [sigilPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "sigil", subcommand] + extraArgs
            }
            if let sigilPath = self.sigilExecutable(searchPaths: searchPaths) {
                return [sigilPath, subcommand] + extraArgs
            }

            let missingEntry = """
            sigil entrypoint missing (looked for dist/index.js or bin/sigil.js); run pnpm build.
            """
    ...
    // Existing callers still refer to sigilCommand; keep it as node alias.
    static func sigilCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real sigil CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/sigil"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/sigil.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/sigil.js"
            node "$PRJ/bin/sigil.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent sigil"
          pnpm --silent sigil \(quotedArgs);
        else
          echo "sigil CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SigilConfigFile.loadDict()

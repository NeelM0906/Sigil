...
private let webChatSwiftLogger = Logger(subsystem: "com.sigil", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SigilChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SigilChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SigilChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SigilChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SigilGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SigilChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SigilChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SigilChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SigilChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SigilChatView>) -> NSViewController
    {

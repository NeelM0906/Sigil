...
enum SigilLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = SigilOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SigilFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SigilLogging.bootstrapIfNeeded()
        let label = SigilLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SigilOSLogHandler: LogHandler {
...
struct SigilFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SigilLogging.parseLabel(self.label)

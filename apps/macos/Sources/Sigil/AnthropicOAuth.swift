...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Sigil token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: SigilOAuthStore.AnthropicOAuthStatus = SigilOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "com.sigil", category: "anthropic-oauth")
...
enum SigilOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let sigilOAuthDirEnv = "SIGIL_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Sigil OAuth token file not found"
            case .unreadableFile: "Sigil OAuth token file not readable"
            case .invalidJSON: "Sigil OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Sigil OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Sigil OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.sigilOAuthDirEnv]?

...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "com.sigil", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Sigil run already has an SSH listener on the expected port (common after restarts),

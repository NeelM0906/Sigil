...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SigilProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "com.sigil", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SigilKit.AnyCodable]()) {
                $0[$1.key] = SigilKit.AnyCodable($1.value.base)
            }
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or SIGIL_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: SigilProtocol.AnyCodable?) -> [String: SigilProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SigilProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SigilKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SigilProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("sigil.control.heartbeat")
    static let controlAgentEvent = Notification.Name("sigil.control.agent")
}

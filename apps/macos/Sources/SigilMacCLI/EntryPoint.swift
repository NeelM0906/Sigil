...
struct SigilMacCLI {
    static func main() async {
        ...
        default:
            fputs("sigil-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      ...
      sigil-mac discover [--timeout <ms>] [--json] [--include-local]
      sigil-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      sigil-mac connect
      sigil-mac connect --url ws://127.0.0.1:18789 --json
      sigil-mac discover --timeout 3000 --json
      sigil-mac wizard --mode local
    """)
}

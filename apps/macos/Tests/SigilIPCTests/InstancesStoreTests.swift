...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SigilProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SigilProtocol.AnyCodable] = [
            "presence": .init([SigilProtocol.AnyCodable(entry)]),
        ]
        let payload = SigilProtocol.AnyCodable(payloadMap)

...
private final class TestChatTransport: @unchecked Sendable, SigilChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SigilChatHistoryPayload]
    private let sessionsResponses: [SigilChatSessionsListResponse]

    private let stream: AsyncStream<SigilChatTransportEvent>
    private let continuation: AsyncStream<SigilChatTransportEvent>.Continuation

    init(
        historyResponses: [SigilChatHistoryPayload],
        sessionsResponses: [SigilChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SigilChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SigilChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SigilChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [SigilChatAttachmentPayload]) async throws -> SigilChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return SigilChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SigilChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SigilChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SigilChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SigilChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SigilChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SigilChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SigilChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SigilChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SigilChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SigilChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }

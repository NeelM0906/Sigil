...
@MainActor
struct SigilChatComposer: View {
    @Bindable var viewModel: SigilChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(SigilChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(SigilChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SigilPendingAttachment.id)
                { (att: SigilPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SigilPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SigilChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(SigilChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(SigilChatTheme.subtleCard)
        .clipShape(Capsule())
    }

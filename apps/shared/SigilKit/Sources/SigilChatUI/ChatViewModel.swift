...
private let chatUILogger = Logger(subsystem: "com.sigil", category: "SigilChatUI")

@MainActor
@Observable
public final class SigilChatViewModel {
    public private(set) var messages: [SigilChatMessage] = []
    ...
    public var attachments: [SigilPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SigilChatPendingToolCall] = []
    public private(set) var sessions: [SigilChatSessionEntry] = []
    private let transport: any SigilChatTransport
    ...
    private var pendingToolCallsById: [String: SigilChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SigilChatTransport) {
    ...
    public var sessionChoices: [SigilChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [SigilChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [SigilChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SigilPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SigilChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SigilChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [SigilChatMessage]) -> [SigilChatMessage] {
        var result: [SigilChatMessage] = []
    ...
    private static func dedupeKey(for message: SigilChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SigilChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> SigilChatAttachmentPayload in
            SigilChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SigilChatSessionEntry {
        SigilChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SigilChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SigilChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SigilAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SigilChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SigilPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
